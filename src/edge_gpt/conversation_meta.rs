...
use crate::edge_gpt::util::random_forwarded_ip;

use super::{util::new_reqwest_client, CookieInFile};

fn create_conversation_headers(forwarded_ip: &str) -> HeaderMap {
...
impl ConversationMeta {
    ...
    pub async fn create(cookies: &[CookieInFile]) -> Result<(ConversationMeta, String)> {
        let uri = "https://edge.churchless.tech/edgesvc/turing/conversation/create"
            .parse()
            .unwrap();
        ...
        let forwarded_ip = random_forwarded_ip();
        let response = new_reqwest_client()
            .cookie_provider(Arc::new(cookie_jar))
            .build()?
            .get(uri)
            .headers(create_conversation_headers(&forwarded_ip))
            .send()
            .await;
        println!("{:?}", response);
        Ok((response?.json().await?, forwarded_ip))
    }
}
